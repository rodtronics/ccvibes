...
- **Lore is a map of pressure**, not a wiki. It should explain _why_ you keep doing this, not _how_ every system works.
...
- **Entry point**: what makes the branch feel _possible_ to the protagonist.
...
Keep cross-links _practical_. No "prophecies". Just cause and effect.
...
## 7. Primordial As An Anchor (Origin, Not Instruction)
...
- Location: `lore/`
- Naming: every file starts with `lore_`.
  - `lore_philosophy.md`: narrative intent + canon guardrails (this file)
  - `lore_<topic>_notes.md`: integration notes (IDs, mappings, unlock beats, cross-links)
  - `lore_<topic>_story.md`: 100% in-voice story text (no mechanics notes)
...
- Write in blocks that can be pasted into `data/modals.json` later without heavy editing.
  - _Tech Note_: Avoid unescaped double quotes (`"`) inside story blocks; use single quotes (`'`) for dialogue or emphasis to prevent JSON syntax errors.
