...
- **Two-panel layout**: JOBS and ACTIVE tabs both show the same unified view. Individual bordered windows: left box cols 0–64 (content 2–63, 62 cols), 1-char gap at col 65, right box cols 66–131 (content 67–130, 64 cols). `listTop = 6` (row 5 = panel top borders with "JOBS"/"ACTIVE RUNS" labels). Focused panel border = `PANEL_SELECTED` teal; unfocused = `DIM_GRAY`. `panelFocus` = 'left'|'right'. `runsFilterMode` = 'matching'|'all'. J focuses left, A focuses right, F toggles filter. Enter on a run opens the run detail modal.
- **Run sorting**: Use `sortRunsActiveFirst` from `js/engine.js` (shared comparator).
