...
STILL NOT WORKING: we should then be able to press right from the options screen where we can see the crimes running in that crime and thats when we can select the different runs. when a run is selected we have two options, X to stop and z to stop after this run. it might need to scroll to let us select the relevant runs
...
when we are in an option and we press right it should move the selection to the active runs under that crime. when one is selected thats when X and Z are visible. and up and down should scroll through them. this will mean implementing a scrolling method, which will be used in a few places. we might need to make our own scroll bars with "|" and "\*" for example. perhaps we can also push another button like P and that gives the same info window that you get before you choose to run a crime (if you select the deeper options) and just tells you who is on the crime and you can choose to stop or forfeit.
